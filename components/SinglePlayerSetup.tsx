...
const SinglePlayerSetup: React.FC<SinglePlayerSetupProps> = ({ onStartGame, onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim()) {
      const playerConfig = { name: name.trim(), characterImg: selectedCharacter.img };

      // Filter out player's character and randomize the rest for bots
      const availableBotChars = CHARACTERS_LIST.filter(c => c.name !== selectedCharacter.name);
      
      // Shuffle available characters using Fisher-Yates algorithm
      ...
      const botsToCreate = availableBotChars.slice(0, numBots);
      const botConfigs = botsToCreate.map(botChar => ({
          name: botChar.name, // Using character name as bot name
          characterImg: botChar.img
      }));

      onStartGame([playerConfig, ...botConfigs]);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={!name.trim()}
              className={`${buttonBaseClass} bg-[#b95c5c] hover:bg-[#a94c4c]`}
            >
              Bắt Đầu
            </button>
          </div>
